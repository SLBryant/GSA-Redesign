<?php include('partials/header.php');?>

        <!-- Navigation -->
        <?php include('partials/inner-main-navigation.php'); ?>
        <?php include('partials/breadcrumb.php');?>

        <header class="landing-page-header" id="sustainability">
            <div class="container">
                <hgroup>
                    <h1 class="col-md-12">Strategically Sustainable</h1>
                    <p class="col-md-6">"GSA will eliminate its impact on the natural environment and use its government-wide influence to reduce the environmental impact of the federal government" - GSA Sustainability Plan</p>
                </hgroup>
            </div>
        </header>

        <div class="container page-content">
            <p class="col-sm-9">The agency unveils ambitious plan to reduce the environmental impact of the federal government and work to eliminate its own carbon footprint. <a href="#">Read the plan</a><br> <br></p>
            <!-- tabbed content -->
            <?php include('partials/full-width-tabs-block.php');?>

            <!-- sidebar -->
            <?php include('partials/sidebar.php');?>
        </div>


<?php include ('partials/footer.php');?>