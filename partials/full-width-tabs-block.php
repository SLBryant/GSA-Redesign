<section class="full-width-tabs-block col-sm-9">
    <!-- Nav tabs -->
    <ul class="nav nav-tabs row" role="tablist">
        <li class="active col-sm-4"><a href="#solutions" role="tab" data-toggle="tab">Sustainable Solutions</a></li>
        <li class="col-sm-4"><a href="#innovation" role="tab" data-toggle="tab">A Proving Ground for Innovation</a></li>
        <li class="col-sm-4"><a href="#promise" role="tab" data-toggle="tab">Fulfilling the Promise</a></li>
    </ul>

    <!-- Tab panes -->
    <div class="tab-content">
        <div class="tab-pane active" id="solutions">
            <header>
                <strong>Click to expand and view opportunities for change available now:</strong>
            </header>
            <!-- start open-close -->
            <div class="menuheader expandable">
                <span class="glyphicon glyphicon-chevron-right"></span><a href="#">Business Solutions for Sustainable Acquisition</a>
            </div>
            <div class="categoryitems">
                Question 1 answer goes here.
            </div>
            <div class="menuheader expandable">
                <span class="glyphicon glyphicon-chevron-right"></span><a href="#">Business Solutions for Sustainable Acquisition</a>
            </div>
            <div class="categoryitems">
                Question 1 answer goes here.
            </div>
            <div class="menuheader expandable">
                <span class="glyphicon glyphicon-chevron-right"></span><a href="#">Business Solutions for Sustainable Acquisition</a>
            </div>
            <div class="categoryitems">
                Question 1 answer goes here.
            </div>
        </div>
        <!-- new tab -->
        <div class="tab-pane" id="innovation">
            <header>
                <strong>Click to expand and view opportunities for change available now:</strong>
            </header>
            <!-- start open-close -->
            <div class="menuheader expandable">
                <span class="glyphicon glyphicon-chevron-right"></span><a href="#">Business Solutions for Sustainable Acquisition</a>
            </div>
            <div class="categoryitems">
                Question 1 answer goes here.
            </div>
        </div>
        <!-- new tab -->
        <div class="tab-pane" id="promise">
            <header>
                <strong>Click to expand and view opportunities for change available now:</strong>
            </header>
            <!-- start open-close -->
            <div class="menuheader expandable">
                <span class="glyphicon glyphicon-chevron-right"></span><a href="#">Business Solutions for Sustainable Acquisition</a>
            </div>
            <div class="categoryitems">
                Question 1 answer goes here.
            </div>
            <div class="menuheader expandable">
                <span class="glyphicon glyphicon-chevron-right"></span><a href="#">Business Solutions for Sustainable Acquisition</a>
            </div>
            <div class="categoryitems">
                Question 1 answer goes here.
            </div>
        </div>
    </div>
</section>